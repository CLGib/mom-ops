...
export default async function DirectorNPSPage() {
  ...
  const npsAvg =
    npsResponseCount > 0
      ? (npsScores.reduce((a, b) => a + b, 0) / npsResponseCount).toFixed(1)
      : null;
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "var(--space-lg)", marginBottom: "var(--space-lg)" }}>
          ...
          <div className="card" style={{ padding: "var(--space-md)", minWidth: "140px" }}>
            ...
            <p style={{ fontSize: "1rem", fontWeight: 600, margin: 0 }}>
              {npsPromoters} / {npsPassives} / {npsDetractors}
            </p>
          </div>
        </div>
      </section>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "var(--space-sm)" }}>
                  <div>
                    ...
                    <span className="form-note" style={{ fontSize: "0.85rem" }}>
                      {formatInCentral(r.created_at)}
                    </span>
                  </div>
                </div>
                {r.comment && (
                  <p style={{ margin: "var(--space-xs) 0 0", fontSize: "0.9rem" }}>{r.comment}</p>
                )}
              </li>
            ))}
          </ul>
