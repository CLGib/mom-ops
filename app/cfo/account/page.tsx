...
export default async function CfoAccountPage() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/login?next=" + encodeURIComponent("/cfo/account"));
  ...
  return (
    <main className="app-shell">
      ...
      <section className="card">
        <h2 className="section-heading">Email and password</h2>
